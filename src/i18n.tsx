...
const FRENCH_TRANSLATIONS: Record<string, string> = {
    ...
    'Game Textures': 'Textures du jeu',
    Terrain: 'Terrain',
    'Terrain texture': 'Texture du terrain',
    Default: 'Par défaut',
    Laboratory: 'Laboratoire',
    'Detecting the local Factorio installation...': 'Détection de l’installation locale de Factorio...',
    'Use textures from the detected local Factorio installation': 'Utiliser les textures de l’installation locale de Factorio détectée',
    'Factorio was not detected. Enable this option to select its installation folder.': 'Factorio n’a pas été détecté. Activez cette option pour sélectionner son dossier d’installation.',
    'Game textures could not be loaded. Enable this option to try again.': 'Les textures du jeu n’ont pas pu être chargées. Activez cette option pour réessayer.',
    'Factorio game textures enabled.': 'Textures du jeu Factorio activées.',
    'Factorio game textures disabled.': 'Textures du jeu Factorio désactivées.',
    'Unable to load Factorio game textures.': 'Impossible de charger les textures du jeu Factorio.',
    English: 'Anglais',
    ...
    Frame: 'Image',
    'ticks/s': 'ticks/s',
    'Lamp inspector': 'Inspecteur de lamp',
    Lamp: 'Lamp',
    Close: 'Fermer',
    'Live animation': 'Animation en direct',
    'Animation paused': 'Animation en pause',
    Pause: 'Pause',
    Play: 'Lecture',
    'Lamp color': 'Couleur de la lamp',
    Red: 'Rouge',
    Green: 'Vert',
    Blue: 'Bleu',
    Hexadecimal: 'Hexadécimal',
    'Hexadecimal color': 'Couleur hexadécimale',
    'Color picker': 'Sélecteur de couleur',
    'Transparent / off in this frame': 'Transparente / éteinte sur cette image',
    'Pause the animation before changing this lamp.': 'Mettez l’animation en pause avant de modifier cette lamp.',
    'The selected color changes only the currently displayed animation frame.': 'La couleur sélectionnée modifie uniquement l’image d’animation actuellement affichée.',
    shown: 'affichées',
    ...
    'Click to place audio controller': 'Cliquez pour placer le contrôleur audio',
    'Audio controller placed. All speakers share the same tick clock.': 'Contrôleur audio placé. Tous les speakers partagent la même horloge de ticks.',
    READY: 'PRÊT',
    Help: 'Aide',
    ...
    'Import MP3, WAV, FLAC, OGG, or another FFmpeg-readable audio file. The app can extract up to four simultaneous pitches per left/right channel.': 'Importez un fichier MP3, WAV, FLAC, OGG ou tout autre audio lisible par FFmpeg. L’application peut extraire jusqu’à quatre hauteurs simultanées par canal gauche/droit.',
    'One decider stores every voice detected at a musical attack or pitch transition; each voice uses one programmable speaker and all speakers share the same tick clock.': 'Un decider stocke toutes les voix détectées lors d’une attaque musicale ou d’une transition de hauteur ; chaque voix utilise un programmable speaker et tous les speakers partagent la même horloge de ticks.',
    'The adaptive detector preserves irregular rhythm and quiet passages. The 1–60 events/s setting is only a maximum density because Factorio runs at 60 ticks/s; it never forces notes onto a periodic grid. 4–8 is recommended.': 'Le détecteur adaptatif conserve le rythme irrégulier et les passages faibles. Le réglage de 1 à 60 événements/s est uniquement une densité maximale, car Factorio fonctionne à 60 ticks/s ; il n’impose jamais les notes sur une grille périodique. Une valeur de 4 à 8 est recommandée.',
    'Use the converted preview to hear the exact local Factorio programmable-speaker samples before export. The desktop app detects common Factorio installations automatically, or you can select the game folder manually; game sounds are read in place and are never copied into the app.': 'Utilisez la préécoute convertie pour entendre les échantillons exacts des programmable speakers de l’installation locale de Factorio avant l’export. L’application de bureau détecte automatiquement les installations courantes de Factorio, ou vous pouvez sélectionner manuellement le dossier du jeu ; les sons sont lus sur place et ne sont jamais copiés dans l’application.',
    ...
    'Choose the primary native instrument for each channel, or use Auto to minimize clipped notes. Additional voices automatically use suitable different timbres when possible.': 'Choisissez l’instrument natif principal de chaque canal ou utilisez Auto pour réduire les notes tronquées. Les voix supplémentaires utilisent automatiquement des timbres différents adaptés lorsque c’est possible.',
    'The exact Factorio sound preview timeline can be scrubbed while playing or paused.': 'La timeline de préécoute exacte des sons Factorio peut être parcourue pendant la lecture ou en pause.',
    ...
    'Erase (E)': 'Effacer (E)',
    Stamps: 'Tampons',
    'Animated media': 'Médias animés',
    'GIF, APNG, static or animated WebP, transparent WebM, video, and TGS are supported. Ratio and alpha are preserved; reductions above the selected definition or 30 FPS require confirmation.': 'Les GIF, APNG, WebP statiques ou animés, WebM transparents, vidéos et TGS sont pris en charge. Les proportions et la transparence sont conservées ; toute réduction dépassant la définition choisie ou 30 FPS demande confirmation.',
    'Animated media import is available in the installed desktop application.': 'L’import de médias animés est disponible dans l’application de bureau installée.',
    'Reading TGS animation…': 'Lecture de l’animation TGS…',
    'Inspecting animated media…': 'Analyse du média animé…',
    'Rendering TGS animation…': 'Rendu de l’animation TGS…',
    'Decoding APNG/WebP with Chromium…': 'Décodage APNG/WebP avec Chromium…',
    'Decoding animated media with FFmpeg…': 'Décodage du média animé avec FFmpeg…',
    'This media exceeds the selected Factorio import limits.': 'Ce média dépasse les limites d’import Factorio sélectionnées.',
    'Convert it with these reduced dimensions and/or FPS?': 'Le convertir avec cette définition et/ou ces FPS réduits ?',
    'Media import cancelled.': 'Import du média annulé.',
    'Unable to decode this animated media.': 'Impossible de décoder ce média animé.',
    'Toss Face (bundled)': 'Toss Face (intégrée)',
    'OpenMoji 17 (color artwork)': 'OpenMoji 17 (illustrations couleur)',
    'Microsoft Fluent Emoji (Flat)': 'Microsoft Fluent Emoji (Flat)',
    'Microsoft Fluent Emoji (Color)': 'Microsoft Fluent Emoji (Color)',
    ...
    'Max events / s': 'Événements max / s',
    'Voices / ch.': 'Voix / canal',
    'Maximum audio events per second': 'Nombre maximal d’événements audio par seconde',
    adaptive: 'adaptatif',
    max: 'max',
    'events/s': 'événements/s',
    'Audio voices per channel': 'Voix audio par canal',
    ...
    'Left primary voice': 'Voix principale gauche',
    'Right primary voice': 'Voix principale droite',
    'Auto (best range)': 'Auto (meilleure plage)',
    'Exact Factorio sound preview': 'Préécoute exacte des sons Factorio',
    'Checking Factorio installation…': 'Recherche de l’installation de Factorio…',
    'Factorio speaker sounds detected': 'Sons des speakers Factorio détectés',
    'Factorio speaker sounds not found': 'Sons des speakers Factorio introuvables',
    'Select Factorio folder': 'Sélectionner le dossier de Factorio',
    'Select another Factorio folder': 'Sélectionner un autre dossier de Factorio',
    'Play converted preview': 'Préécouter la conversion',
    'Pause preview': 'Mettre la préécoute en pause',
    'Resume preview': 'Reprendre la préécoute',
    'Stop preview': 'Arrêter la préécoute',
    'Seek converted preview': 'Se déplacer dans la préécoute convertie',
    'Loading game sounds…': 'Chargement des sons du jeu…',
    'Unable to play the converted preview.': 'Impossible de lire la préécoute convertie.',
    'Unable to use this Factorio folder.': 'Impossible d’utiliser ce dossier Factorio.',
    'The preview uses the exact local Factorio programmable-speaker samples and does not copy them into the application.': 'La préécoute utilise les échantillons exacts des programmable speakers de l’installation locale de Factorio et ne les copie pas dans l’application.',
    'Exact Factorio preview is available in the installed desktop application.': 'La préécoute exacte de Factorio est disponible dans l’application de bureau installée.',
    ...
    'Move audio controller': 'Déplacer le contrôleur audio',
    'Place audio controller on grid': 'Placer le contrôleur audio sur la grille',
    'The audio is ready. Place its controller on the grid before it can be exported.': 'L’audio est prêt. Placez son contrôleur sur la grille avant de pouvoir l’exporter.',
    ...
    'This structure requires more animation frames than the current limit.': 'Cette structure nécessite plus d’images d’animation que la limite actuelle.',
    Structure: 'Structure',
    'Required frames': 'Images requises',
    'Current frame limit': 'Limite actuelle d’images',
    OK: 'OK',
    'raise the frame limit and continue': 'augmenter la limite d’images et continuer',
    'cancel this structure': 'annuler cette structure',
    'Structure cancelled.': 'Structure annulée.',
    'Animated text stamp': 'Tampon de texte animé',
    'Animated emoji': 'Emoji animé',
    'Blueprint animation': 'Animation de blueprint',
    ...
    'Static emoji library': 'Bibliothèque d’emojis statiques',
    'Emoji library': 'Bibliothèque d’emojis',
    'Selected emoji destination': 'Destination de l’emoji sélectionné',
    'Insert into text field': 'Insérer dans le champ de texte',
    'Create a direct grid stamp': 'Créer un tampon directement sur la grille',
    'Animated emoji attached to STAMPS text': 'Emoji animé attaché au texte STAMPS',
    'The emoji is inserted at the text cursor and keeps its real animation frames when the STAMPS text is created.': 'L’emoji est inséré à l’emplacement du curseur et conserve ses véritables frames d’animation lors de la création du texte STAMPS.',
    'System and bundled emoji fonts': 'Polices d’emojis système et intégrées',
    'Downloadable artwork libraries': 'Bibliothèques d’illustrations téléchargeables',
    'The selected library is used for both the catalog preview and the created stamp.': 'La bibliothèque sélectionnée est utilisée à la fois pour l’aperçu du catalogue et pour le tampon créé.',
    ...
    'Twemoji 17 (static artwork)': 'Twemoji 17 (illustrations statiques)',
    'Twemoji uses downloadable static artwork saved in the persistent emoji cache. The official animated catalog uses Google Noto animations.': 'Twemoji utilise des illustrations statiques téléchargées puis enregistrées dans le cache persistant d’emojis. Le catalogue animé officiel utilise les animations Google Noto.',
    ...
    'and never exceeds 30 FPS.': 'et ne dépasse jamais 30 FPS.',
    Text: 'Texte',
    'FFmpeg detects musical attacks and pitch transitions at adaptive tick positions, with up to four simultaneous pitches per left/right channel. The selected 1–60 events/s value is a density ceiling, not a forced rhythm; 4–8 is recommended.': 'FFmpeg détecte les attaques musicales et les transitions de hauteur à des ticks adaptatifs, avec jusqu’à quatre hauteurs simultanées par canal gauche/droit. La valeur choisie de 1 à 60 événements/s est un plafond de densité, pas un rythme imposé ; une valeur de 4 à 8 est recommandée.',
    'Detecting adaptive polyphonic note events with FFmpeg...': 'Détection des événements de notes polyphoniques adaptatifs avec FFmpeg…',
    'Auto selects the native instruments whose pitch ranges clip the fewest detected notes. Equivalent ranges use different timbres when possible; each voice keeps one instrument for the whole track.': 'Auto sélectionne les instruments natifs dont les plages tronquent le moins de notes détectées. Les plages équivalentes utilisent des timbres différents lorsque c’est possible ; chaque voix conserve le même instrument pendant toute la piste.',
...
const DYNAMIC_TRANSLATIONS: DynamicTranslation[] = [
    ...
    { english: /^(\d[\d\s,.]*) polyphonic note events extracted$/, french: /^(\d[\d\s,.]*) événements de notes polyphoniques extraits$/, toFrench: '$1 événements de notes polyphoniques extraits', toEnglish: '$1 polyphonic note events extracted' },
    { english: /^(\d[\d\s,.]*) polyphonic events extracted\. Click the grid to place the audio controller\.$/, french: /^(\d[\d\s,.]*) événements polyphoniques extraits\. Cliquez sur la grille pour placer le contrôleur audio\.$/, toFrench: '$1 événements polyphoniques extraits. Cliquez sur la grille pour placer le contrôleur audio.', toEnglish: '$1 polyphonic events extracted. Click the grid to place the audio controller.' },
