...
- Activer **Textures du jeu** dans le header pour afficher les lamps, combinators, poles, roboports, speakers et display panels visibles avec leurs sprites d’entités posées, lus directement dans l’installation locale de Factorio détectée. Les fonds colorés des cases disparaissent dans ce mode : seules les lamps et leur lueur en temps réel reçoivent la couleur RGB courante. Le sélecteur de terrain peut conserver le fond de l’éditeur ou utiliser les tuiles natives de Nauvis et du Laboratory. Les véritables sprites des lamps restent visibles beaucoup plus loin ; seules les vues exceptionnellement denses passent à un aperçu compact qui conserve les couleurs au lieu d’un masque d’occupation gris. Les installations Steam et autonomes sont détectées automatiquement, et la case ouvre un sélecteur du dossier du jeu si la détection échoue. Chaque asset manquant revient individuellement à l’aperçu vectoriel.
...
- Déplacer continuellement la grille avec les flèches ou le groupe physique WASD/ZQSD. Chromium détecte automatiquement les libellés du clavier lorsque le système d’exploitation expose sa disposition.
